...
public class StringRandomizer {
  ...
  /**
   * Function generates a random string of length 0 to 27 with characters a-z (all cases), and
   * spaces
   *
  ...
  public String generateRandomString() {
    ...
    String characters =
        "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ "; // define the characters to use
    StringBuilder sb = new StringBuilder(length);
    for (int i = 0; i < length; i++) {
      int index =
          new Random()
              .nextInt(
                  characters.length()); // generate a random index within the range of the character
      // string
